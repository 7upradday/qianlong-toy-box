window.preparePhysicalCabinet=(()=>{
  let scenePromise;
  return()=>scenePromise||(scenePromise=(async()=>{
  try {
    await import('./cabinet-physical.js?v=fast-2');
    if(!window.physicalCabinet)throw window.cabinetSceneError||new Error('Scene initialization failed');
    return window.physicalCabinet;
  }catch(error){
    ...
    window.cabinetApp?.renderFallback();
    throw error;
  }
  })());
})();
